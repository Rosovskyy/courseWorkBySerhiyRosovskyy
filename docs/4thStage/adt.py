...
class InvalidSymbolError(Exception):
    """ Class for invalib symbol error representation """
    pass
...
class GoogleMapPlotter(object):
    ...
    def __init__(self, center_lat, center_lng, zoom, apikey=''):
        """
        (GoogleMapPlotter, number, number, number, str) -> Nonetype
        Sets the needed parameters of the class
        """
    ...
    def from_geocode(cls, location_string, zoom=13):
        """ (GoogleMapPlotter, str, number) -> GoogleMapPlotter
        Get the location and return the
    ...
    def geocode(self, location_string):
        """
        (GoogleMapPlotter, str) -> tuple
        The function gets the street and returns
        the location of it.
        """
    ...
    def marker(self, lat, lng, color='#FF0000', c=None, title="no implementation"):
        """
        (GoogleMapPlotter, number, number) -> Nonetype
        The function takes coordinates and append
        them to the list of points, which will be
        appeared on the map.
        """
        if c:
            color = c
    ...
    def draw(self, htmlfile):
        """Create the html file which include one
        google map and all points and paths. If
